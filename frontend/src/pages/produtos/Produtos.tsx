...
export function Produtos() {
  ...
  const loadProdutos = async () => {
    ...
    setError(null)
    try {
      // lista de metricas - coloquei limit 1000 pra nao ficar pesado se tiver muito produto
  ...
  // Aqui calculei os kpis baseados na lista de metricas 
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="min-h-[300px] flex flex-col items-center justify-center p-6 bg-error-50 rounded-xl border border-error-100 max-w-2xl mx-auto text-center space-y-4">
          ...
          <Button onClick={loadProdutos} className="bg-error hover:bg-error-400 text-white">
            Tentar novamente
          </Button>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <ProductCardKPI
              ...
              value={formatNumber(totalActiveProducts)}
              icon={Package}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            {filteredProdutos.length === 0 ? (
              ...
              <>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {paginatedProdutos.map((prod) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4.5 font-medium text-gray-700">{formatNumber(prod.quantidade_vendida)}</td>
                ...
                {/* Seção de Controle de Paginação (Até 10 itens por página) */}
                {totalPages > 1 && (
                  <div className="bg-white px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3 select-none">
                    <div className="text-caption text-gray-500 font-medium order-2 sm:order-1">
                      ...
                      de <span className="font-semibold text-gray-700">{filteredProdutos.length}</span> produtos
                    </div>
                    <div className="flex items-center gap-1 order-1 sm:order-2">
                      <button
                        onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                        disabled={currentPage === 1}
                        className="p-1.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-100 active:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-150"
                      ...
                      {getPageNumbers().map((page, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`h-8 w-8 rounded-lg text-caption font-semibold transition-all duration-150 ${currentPage === page
