...
| Camada | Descrição |
|--------|-----------|
| 📥 **Landing** | Ingestão de dados transacionais, cadastrais e logs de navegação em formato bruto (`.csv`) |
| 🥉 **Bronze** | Leitura automatizada via Spark e persistência em Delta Table sem alterações morfológicas, preservando o histórico exato da fonte |
| 🥈 **Silver** | Purificação estrutural: limpeza, padronização de strings, Regex, conversões de timestamps, tratamento de nulos e desmembramento de IDs |
| 🥇 **Gold** | Modelagem Star Schema com tabelas Dimensão (Cliente, Produto) e Fato (Vendas, Suporte, Avaliações, Eventos). Destaque para `fato_360_cliente`, que agrega KPIs como LTV, recência, NPS e tickets |
| 🚀 **Entrega** | Exportação para SQLite local (zero-setup/serverless), orquestrada por DAG com dependências seguras executada diariamente à meia-noite |
